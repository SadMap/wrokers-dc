  ...
  async fetch(
    ...
    if (subdomain === "www" || (subdomain === "akp" && path === "/")) {
      const html = await fetch(
        "https://raw.githubusercontent.com/SadMap/wrokers-dc/main/index.html"
      );
    ...
    if (subdomain === "akp") {
      ...
      if (path === "/shorten" && request.method === "POST") {
        ...
        if (!secret) {
          return new Response("Secret is required", {
            status: 401,
            headers: {
              "content-type": "text/plain;charset=UTF-8",
              "Access-Control-Allow-Origin": "*",
        ...
        // Check if url is not used
        const redirect = await env.redirects.get(`${subdomain}/${url}`);
        if (redirect) {
          return new Response(`Redirect already exists`, {
            status: 409,
            headers: {
              "content-type": "text/plain;charset=UTF-8",
              "Access-Control-Allow-Origin": "*",
        ...
        return new Response(
          `Successfully created redirect for ${subdomain}.akp.bar/${url}`,
          {
            status: 200,
            headers: {
              "content-type": "text/plain;charset=UTF-8",
      ...
      if (path === "/delete" && request.method === "POST") {
        ...
        return new Response(
          `Successfully deleted redirect for ${subdomain}.akp.bar/${url}`,
          {
            status: 200,
          }
        );
      ...
      if (
        path === "/shorten" ||
        (path === "delete" && request.method === "OPTIONS")
      ) {
    ...
    const isBot =
      request.headers.get("user-agent")?.includes("Discordbot/2.0;") ||
      request.headers
        .get("user-agent")
        ?.includes("(Macintosh; Intel Mac OS X 11.6; rv:92.0)");
